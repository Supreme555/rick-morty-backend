...
import { toCharacterSummary, toEpisodeSummary, toPaginated } from '../rick-api/mappers.js';
import type { RawCharacter, RawEpisode } from '../rick-api/raw.types.js';
import type { EpisodeDetail, EpisodeSummary, Paginated } from '../common/types.js';
...
export class EpisodesService {
  ...
  async list(query: ListEpisodesDto): Promise<Paginated<EpisodeSummary>> {
    ...
    const raw = await this.api.list<RawEpisode>('episode', { page, ...filters });
  ...
  async getById(id: number): Promise<EpisodeDetail> {
    ...
    const characters = await this.api.getMany<RawCharacter>('character', summary.characterIds);
