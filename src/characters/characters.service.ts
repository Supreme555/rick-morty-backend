...
import type { CharacterDetail, CharacterSummary, Paginated } from '../common/types.js';
...
export class CharactersService {
  ...
  async list(query: ListCharactersDto): Promise<Paginated<CharacterSummary>> {
    ...
    const raw = await this.api.list<RawCharacter>('character', { page, ...filters });
  ...
  async getById(id: number): Promise<CharacterDetail> {
    ...
    const episodes = await this.api.getMany<RawEpisode>('episode', summary.episodeIds);
